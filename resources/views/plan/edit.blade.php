@extends('layouts.app', ['title' => __('Installations de Parking')],['activePage' => 'plan'])
                    ...
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 text-right">
                                <a href="{{ route('plan.index') }}" class="btn btn-sm btn-primary">{{ __('Retour à la liste') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                    ...
                    <input type="hidden" name="product_id" value="{{$plan->product->id}}">
                               ...
                               <div class="row">
                                   <div class="col-3">
                                    ...
                                    <label class="form-control-label" for="amount">{{ __('montant') }}</label>
                                   ...
                                   <div class="col-6">
                                    ...
                                    <label class="form-control-label" for="interval">{{ __('Intervalles') }}</label>

                                    <select name="interval" class="form-control form-control-alternative{{ $errors->has('interval') ? ' is-invalid' : '' }}" required>
                                        <option value='day' {{$plan->interval =='day' ? 'selected' : ''}}>{{ __('Jour')}}</option>
                                        <option value='week' {{$plan->interval =='week' ? 'selected' : ''}}>{{ __('Semaine')}}</option>
                                        <option value='month' {{$plan->interval =='month' ? 'selected' : ''}}>{{ __('Mois')}}</option>
                                        <option value='year' {{$plan->interval =='year' ? 'selected' : ''}}>{{__('Année')}}</option>
                                ...
                                <div class="text-center">
                                    <button type="submit" class="btn btn-primary rtl_btn mt-4">{{ __('Sauvegarder') }}</button>
                                </div>
                            </div>
                        </form>
                    </div>
